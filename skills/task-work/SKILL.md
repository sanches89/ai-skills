...
states is met.
The output is the change in the working tree plus a work report that holds
only what the caller needs to continue the rest of the work.

## Terms

These words have exactly one meaning in this skill.

- **Task**: one unit of work. When the `task-create` or `task-breakdown` skill
  wrote it, it is in the task format: Summary, Success criteria, Scope,
  Approach, Decisions, Context, Subtasks, Verification.
- **Subtask**: one commit-sized unit of work inside a task. When the
  `task-breakdown` skill wrote it, it is in the subtask format: Task, Depends
  on, Goal, Context, Changes, Acceptance criteria, Verification.
...
- **Source**: where the target came from. Exactly one of: *server* (an item),
  *file* (a task file or a subtask file), or *text*. The source decides how the
  chain is built in Step 2.
...
- **Depends on**: the earlier subtasks a subtask needs merged first.
...
3. **Stay in scope.** Change only what the target's Changes or Approach section
   names. A file the target does not name changes only when a named change
   does not build or pass without it, and that file is recorded as a
   deviation. Test files that hard rule 5 requires are in scope and are not
   deviations. Never do what any task in the chain lists under *Out of scope*.
   Never do work that belongs to a sibling subtask.
...
5. **Tests ship with the change.** When the project has a test setup, every
   behavior the change adds or alters gets a test in the same change, whether
   or not the target names tests. The tests the target names are the minimum,
   never the limit. When the project has no test setup, write no tests and
   install no test framework.
6. **Never ask what research can answer.** The chain, the code, the docs, and
   the connected tools are consulted before the first question.
7. **Never assume.** If a decision changes the work and research cannot settle
   it, ask the user.
8. **One question at a time.** Every question is written in chat using the
   *Question format* below, then the turn ends and waits for the answer. Never
   ...
   `AskUserQuestion`). Questions are plain chat text. When the caller is an
   agent, the caller relays the question to the user and passes the answer
   back.
...
- **An item identifier or URL** (for example `PAY-212`, `#128`, an issue link).
  Source: *server*. Fetch the item and its children. To find the server, list
  the MCP servers and tools available to the agent. In Claude Code, MCP tools
  are deferred, so search them with `ToolSearch` using keywords like
  `issue ticket project linear jira notion asana github`. If no server is
  connected, ask one question using the question format: give the target as a
...
has children, or when its Subtasks section holds entries other than `None.`

### Step 2: Build the chain

Walk from the target to the root task:
- Source *file*: the parent of a subtask file is the `task.md` in the same task
  folder, which its `Task` line links to. A task file has no parent.
- Source *server*: the parent of an item is the item that the server's parent
  relation points to, or the item that its `Task` line links to when the server
  has no parent relation. Fetch it, then fetch its parent, until an item has no
  parent. Stop the walk when an identifier repeats.
...
- its Decisions and the conventions it states;
- its *Out of scope* list;
- the success criteria that the target contributes to;
- its Subtasks list, the target's position in it, and the title and Goal of
...
ask one question using the question format: which of the two statements holds.
...
acceptance criteria, success criteria, or definition of done. When it has no
such list, write the criteria from its text, each observable and binary, and
ask the user to confirm them with one question using the question format.
...
and Changes or Approach exists. Read the code that changes and the code that
calls it, not only file names. When a named path or symbol is gone, search for
where it moved:
- exactly one match: use it and record a deviation;
- no match, or more than one: ask one question using the question format.
...
Use a documentation server such as Context7 when one is connected. Otherwise
read the installed package's own docs and types.
...
copied word for word, then one entry for the target's Verification, then one
entry per command from 4c:
...
access the agent lacks keeps the proof `none`, and the result is `blocked`
...
- write each test before the code that makes it pass. Run it with the command
  from 4e and confirm it fails for the expected reason: the missing behavior,
  which includes a symbol that does not exist yet, and never a mistake in the
  test itself. Then write the code and run the test again;
...
- a refactor of code that no existing test covers gets tests first. They pass
  before the refactor and after it. Each is seen failing once by breaking the
  asserted behavior and restoring it, as `references/unit-testing.md` states;
- edit an existing test only when the target changes the behavior it asserts.
  Every other failing test is fixed in the code, by hard rule 4.

When a decision is missing, research first. When research cannot settle it, ask
one question using the question format. Record the answer in the Step 2 notes
and, in the work report, under *Affects other work*.

### Step 7: Verify

Run, in this order:
1. the target's Verification;
...
result, and tick the entry only when it passes. On any failure, fix the cause
inside the target's scope and run this whole step again from the start, because
a fix can break an earlier check. Evidence from a run made before the last edit
does not count.
...
- every changed file is named in the target's Changes or Approach, or is
  recorded as a deviation with its reason. Revert every other change;
