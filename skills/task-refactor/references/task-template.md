...
The task format is the same one the `task-create` and `task-breakdown` skills
write. The `task-work` skill implements it. Fill every section with what the
...
- Links: in the draft, name titles and numbers only on `Task`, `Depends on`,
  and in the Subtasks list. Step 10 replaces them with file links or item
  links at save time.
